function Q=applyLoadsTriang(nodes,elem,h,nodLoads,Q, forceLoad)
%------------------------------------------------------------------------
...
for k=1:numElem
  ...
  for inod=1:3 %loop for the three nodes
      r=find(nodLoads==elem(k,inod)); %find if one node of element k is in 
                                      %the convection node list
      if(~isempty(r))
  ...
  if ( ij(1) > 0) %it's an existing edge
    ...
    Q(posit)=Q(posit)+0.5*h*L*forceLoad; %same (x,y) force on both nodes
